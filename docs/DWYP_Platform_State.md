...
- **Pending template sweep (2026-06-04, now due — all four companions built):** Three `# Companion Voice` edits due: (1) firewall produce/redirect nuance — when does the companion produce copy vs. redirect to another surface; (2) quote attribution handling — companion should not automatically append attribution lines to quote output (human judgment call); (3) Schedule authority-order rule — DWYP analytics > trends cache > general craft; companion names which it is using. All three deferred to a single hub-authored template sweep.
