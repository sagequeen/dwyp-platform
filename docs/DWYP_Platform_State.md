...
- **Reels sub-tab (Design) — live (2026-05-24).** Drive `/preview` playback in phone frame. Caption box reads `Caption_Host`; Generate writes caption from `Reel_Summary` via Claude (`generateReelCaption` — system prompt from `# Caption Mechanics` + `# Voice Prohibitions`). Three verbs: Export (moves reel → `Manual_Exports/`, paired `.txt`), Edit with Vids (spawns `edit_vids` task stub), Request Revision (§4 atomic close — completes open `Review_Reels` task, spawns `Revise_Reels` with `Asset_ID` FK). `closeReelRevision`: swaps `Drive_File_ID` on AL row by `Asset_ID`, moves old file → `Reels/Superseded/`, completes open `Revise_Reels` task, `bumpVersion` both domains, next Pulse re-spawns `Review_Reels`.
...
| # | Area | Issue | Action |
|---|---|---|---|
| 15 | `dwyp_ui.html` | Reel player playing-state optimization. `/preview` iframe live; poster state shows Drive chrome — accepted per hub. CSS `top:-52px / height:calc(100%+100px)` clips header in playing state. Stop condition: do not chase both-states-perfect (cross-origin-unreachable). | Pixel-nudge the playing state if needed; poster is out of scope. |
...
**Active approach:** Drive `/preview` iframe, playing-state optimized. `getReelStreamUrl(fileId)` calls `setSharing(ANYONE_WITH_LINK, VIEW)` and returns `https://drive.google.com/file/d/{fileId}/preview`. Frontend uses `<iframe id="stReelPlayerFrame" allowfullscreen>`. URL cached on `reel._streamUrl` — first tap pays the GAS round-trip, every subsequent tap is instant.

**CSS approach:** Phone frame container (`overflow: hidden; position: relative`). Iframe positioned `top: -52px; height: calc(100% + 100px)` — clips Drive header chrome in the playing state. Poster state (pre-tap thumbnail) is visually imperfect; that is accepted. **Stop condition: do not attempt to fix both states. Playing state is solvable; both-states-perfect is cross-origin-unreachable.**
