...
| # | Area | Issue | Action |
|---|---|---|---|
| 15 | `dwyp_ui.html` | **Reels selector temporarily disabled (2026-05-22).** `stSetDesignTab` guard redirects `reels` → `qg`; button `disabled`. Drive `/preview` iframe approach does not fill the phone frame cleanly. Engineering Notes prescribe native `<video>` + `getReelStreamUrl()` (sets sharing, returns `uc?id=` URL) — but `getReelStreamUrl` was removed with Publish retirement. | Re-enable when `getReelStreamUrl` is rebuilt in `dwyp_app.gs` and `stSelectRailReel` is updated to use native `<video>`. Remove the `stSetDesignTab` guard and `disabled` attribute on re-enable. |
