...
| 9 | Revise Reels | Audra | JT comment submit in Reels sorter or §4 Request Revision | ✅ Live. Spawned by `requestReelRevision` (§4 close) and `spawnReelEditTask`. Carries `Asset_ID` FK to Asset_Library. Revise_Episode for Audra still needed (F-4). |
...
| # | Area | Issue | Action |
|---|---|---|---|
| 15 | `dwyp_ui.html` | **RESOLVED (2026-05-24).** Selector re-enabled with Drive `/preview` playback. `stSetDesignTab` guard and `disabled` attribute removed. Native `<video>` + GCS hosting deferred — not a reblocker. | — |
